    ...
    return (
        <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'py-4' : 'py-8'}`}>
            <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
                ...
                {/* Logo */}
                <Link to="/" onClick={() => window.scrollTo(0, 0)} className={`flex items-center gap-2 p-2 rounded-xl transition-all cursor-pointer ${scrolled ? 'bg-white/80 dark:bg-primary-dark/80 backdrop-blur-md border border-slate-200 dark:border-white/5 shadow-sm' : ''}`}>
                ...
                <div className={`hidden md:flex items-center gap-8 px-8 py-3 rounded-full border transition-all ${scrolled ? 'bg-white/80 dark:bg-primary-dark/80 backdrop-blur-md border-slate-200 dark:border-white/5 shadow-lg' : 'bg-transparent border-transparent'}`}>
                    {['Approach', 'Services', 'Work', 'About'].map(item => (
                        <a key={item} href={location.pathname === '/' ? `#${item.toLowerCase()}` : `/#${item.toLowerCase()}`} className="text-sm font-medium text-primary-dark/70 dark:text-primary-light/80 hover:text-accent-pop dark:hover:text-white transition-colors">
                            {item}
                        </a>
                    ))}
                </div>
                ...
                {isOpen && (
                    ...
                    >
                        <div className="flex flex-col p-6 space-y-6">
                            {['Approach', 'Services', 'Work', 'About'].map(item => (
                                <a key={item} href={location.pathname === '/' ? `#${item.toLowerCase()}` : `/#${item.toLowerCase()}`} className="text-2xl font-light text-primary-dark dark:text-white" onClick={() => setIsOpen(false)}>
                                    {item}
                                </a>
                            ))}
                        </div>
